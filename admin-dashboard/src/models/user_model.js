...
const UserModel = {
    ...
    findByLoginIdentifier: async (identifier, connection = null) => {
        const sql = `SELECT id_nguoi_dung, email_dang_nhap, ten_dang_nhap, mat_khau_bam, vai_tro, avartar as url_anh_dai_dien, dang_hoat_dong
                     FROM nguoidung
    ...
    findById: async (id_nguoi_dung, connection = null) => {
        const sql = `SELECT id_nguoi_dung, email_dang_nhap, ten_dang_nhap, vai_tro, avartar as url_anh_dai_dien,
    ...
    create: async (userData, connection = null) => {
        const {
            ten_dang_nhap, // Sẽ là id_nguoi_dung theo logic mới nếu id_nguoi_dung là VARCHAR
            email_dang_nhap,
            mat_khau_bam,
            vai_tro = 'user',
            url_anh_dai_dien = null, // Sẽ map vào cột `avartar`
            dang_hoat_dong = 1 // Mặc định là đang hoạt động
        } = userData;
        // Nếu id_nguoi_dung là INT AUTO_INCREMENT thì không cần truyền vào INSERT
        const sql = `INSERT INTO nguoidung
                        (email_dang_nhap, ten_dang_nhap, mat_khau_bam, vai_tro, avartar, dang_hoat_dong, ngay_tao, ngay_cap_nhat)
                     VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`;
        try {
            const [result] = await UserModel._query(sql, [
                email_dang_nhap, ten_dang_nhap, mat_khau_bam, vai_tro,
                url_anh_dai_dien, dang_hoat_dong
            ], connection);
            return { id_nguoi_dung: result.insertId, ...userData }; // Lấy ID tự tăng
        } catch (error) {
    ...
    findAll: async ({ limit, offset, searchTerm, sortBy = 'ngay_tao', order = 'DESC' }, connection = null) => {
        ...
        const dataSql = `SELECT id_nguoi_dung, ten_dang_nhap, email_dang_nhap, vai_tro, dang_hoat_dong,
                                avartar as url_anh_dai_dien, lan_dang_nhap_cuoi, ngay_tao, ngay_cap_nhat
        ...
        try {
            ...
            return { users: rows, totalItems: countRows[0].totalItems };
        } catch (error) {
    ...
    update: async (id_nguoi_dung, userData, connection = null) => {
        const fields = [];
        const values = [];
        // Các trường cho phép cập nhật từ admin
        const allowedFields = ['email_dang_nhap', 'ten_dang_nhap', 'vai_tro', 'avartar', 'dang_hoat_dong'];

        Object.keys(userData).forEach(key => {
            // Map url_anh_dai_dien to avartar
            const dbKey = key === 'url_anh_dai_dien' ? 'avartar' : key;

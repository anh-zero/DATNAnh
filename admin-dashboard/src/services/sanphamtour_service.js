...
const TourService = {
    ...
    updateTour: async (id_san_pham_tour, tourData, file) => {
        ...
        try {
            ...
            } else if (tourData.url_anh_bia === null) {
                tourUpdatePayload.url_anh_bia = null;
            }
            // Remove partners from tourUpdatePayload if it's not stored directly with sanphamtour
            delete tourUpdatePayload.partners;

            await TourModel.update(id_san_pham_tour, tourUpdatePayload, connection);

            // Partner association logic is removed from here.
            // If partners need to be updated, it would be through managing lichtrinhtour and their dichvutour entries.

            await connection.commit();
            const updatedTour = await TourModel.findById(id_san_pham_tour); // Fetch the updated tour
    ...
    getTourStatistics: async () => {
        try {
            ...
            console.error("Error in TourService.getTourStatistics:", error);
            throw error;
        }
    },
